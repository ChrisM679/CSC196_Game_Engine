...
namespace viper {
    ...
    public:
		...
		bool GetPrevMouseButtonDown(uint8_t button) const { assert(button < 3); return m_prevMouseButtonState[button]; }
		bool GetMouseButtonPressed(uint8_t button) const { assert(button < 3); return !m_prevMouseButtonState[button] && m_mouseButtonState[button]; }
		bool GetMouseButtonReleased(uint8_t button) const { assert(button < 3); return m_prevMouseButtonState[button] && !m_mouseButtonState[button]; }
